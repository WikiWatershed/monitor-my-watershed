...
class SiteTypeSelect(forms.Select):
    site_types = {
        name: definition
        for (name, definition) in SiteType.objects.filter(
            name__in=allowed_site_types
        ).values_list("name", "definition")
    }

    def create_option(
        self, name, value, label, selected, index, subindex=None, attrs=None
    ):
        option = super(SiteTypeSelect, self).create_option(
        ...
        if isinstance(value, forms.models.ModelChoiceIteratorValue):
            value = value.value
        option["attrs"]["title"] = (
            self.site_types[value] if value in self.site_types else ""
        )
        return option


class SampledMediumField(forms.ModelChoiceField):
    custom_labels = {"Liquid aqueous": "Water - Liquid Aqueous"}

    @staticmethod
    def get_custom_label(medium):
        return (
            SampledMediumField.custom_labels[medium]
            if medium in SampledMediumField.custom_labels
            else medium
        )
...
class OrganizationForm(forms.ModelForm):
    ...
    organization_type = forms.ModelChoiceField(
        queryset=OrganizationType.objects.all().exclude(
            name__in=["Vendor", "Manufacturer"]
        ),
        required=False,
        help_text="Choose the type of organization",
    )
...
class SiteSensorForm(forms.ModelForm):
    ...
    sensor_manufacturer = forms.ModelChoiceField(
        queryset=Organization.objects.only_vendors(),
        label="Sensor Manufacturer",
        help_text="Choose the manufacturer",
        to_field_name="organization_code",
    )
    sensor_model = forms.ModelChoiceField(
        queryset=EquipmentModel.objects.all(),
        label="Sensor Model",
        help_text="Choose the model of your sensor",
    )
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.all(),
        label="Measured Variable",
        help_text="Choose the measured variable",
    )
    unit = forms.ModelChoiceField(
        queryset=Unit.objects.all(),
        label="Units",
        help_text="Choose the measured units",
    )
    sampled_medium = SampledMediumField(
        queryset=Medium.objects.filter(pk__in=allowed_sampled_medium),
...
class SiteAlertForm(forms.ModelForm):
    notify = forms.BooleanField(
        required=False,
        initial=False,
        label="Notify me if site stops receiving sensor data.",
    )
    hours_threshold = forms.DurationField(
        required=False, label="Notify after", widget=NumberInput(attrs={"min": 1})
    )
