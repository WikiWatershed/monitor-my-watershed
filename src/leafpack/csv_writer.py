...
class LeafPackCSVWriter(object):
    ...
    def filename(self):
        ...
        return "{}_{}_{:03d}.csv".format(
            self.site.sampling_feature_code,
            self.leafpack.placement_date,
            int(self.leafpack.id),
        )
    ...
    def write(self):
        ...
        self.writerow(
            [
                "Site Description",
                site_registration.sampling_feature.sampling_feature_description,
            ]
        )
        ...
        self.writerow(
            ["Vertical Datum", site_registration.sampling_feature.elevation_datum]
        )
        ...
        self.writerow(
            [
                "URL",
                "{0}/sites/{1}/".format(
                    self.HYPERLINK_BASE_URL, site_registration.sampling_feature_code
                ),
            ]
        )
        ...
        for lpg in (
            LeafPackBug.objects.filter(leaf_pack=leafpack)
            .order_by("bug__pollution_tolerance")
            .order_by("-bug__sort_priority")
        ):
            if lpg.bug.family_of is not None:
                # prepend a '-' to suborder taxons
                taxon_name = " - {0} ({1})".format(
                    lpg.bug.common_name.title(), lpg.bug.scientific_name.title()
                )
            else:
                taxon_name = "{0} ({1})".format(
                    lpg.bug.common_name.title(), lpg.bug.scientific_name.title()
        ...
        self.writerow(
            ["Total number of individuals found", str(leafpack.taxon_count())]
        )
        ...
        self.writerow(
            [
                ...
                leafpack.PollutionToleranceIndexRating(PTI_score=PTI_score),
            ]
        )
